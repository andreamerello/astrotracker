...
int i2c_write(uint8_t i2c_addr, uint8_t reg, int len, unsigned char *buf)
{
	int ret;
	char *tmp = alloca(len + 1);

	if ((int)i2c_addr != last_i2c_addr) {
		last_i2c_addr = i2c_addr;
		if (ioctl(i2c_file, I2C_SLAVE, i2c_addr) < 0)
			perror("i2c slave addr select failed (wr)\n");
	}

	tmp[0] = reg;
	memcpy(tmp + 1, buf, len);

	ret = write(i2c_file, tmp, len + 1);
	if (ret != len + 1) {
...
int i2c_read(uint8_t i2c_addr, uint8_t reg, int len, unsigned char *buf)
{
	char tmp = reg;
	...
	if ((int)i2c_addr != last_i2c_addr) {
		last_i2c_addr = i2c_addr;
		if (ioctl(i2c_file, I2C_SLAVE, i2c_addr) < 0)
			perror("i2c slave addr select failed (rd)\n");
	}

	if (write(i2c_file, &tmp, 1) != 1) {
		perror("i2c addr wr failed (rd)\n");
	}

	if (read(i2c_file, buf, len) != len) {
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    data |= st.chip_cfg.gyro_lpf_by & 1;
...
/**
 *  @brief      Set digital low pass filter bypass.
 *  @param[in]  lpf 1: bypass on, 0: bypass off
 *  @return     0 if successful.
 */
int mpu_get_lpf_by(unsigned short *lpf_by)
{
	//unsigned char data;
    ...
    *lpf_by = st.chip_cfg.gyro_lpf_by;

    return 0;
}

int mpu_set_lpf_by(unsigned short lpf_by)
{
    unsigned char data;

    if (lpf_by == st.chip_cfg.gyro_lpf_by)
	    return 0;

    data = st.chip_cfg.gyro_fsr << 3;
    data |= lpf_by & 1;

    if (i2c_write(st.hw->addr, st.reg->gyro_cfg, 1, &data))
        return -1;

    st.chip_cfg.gyro_lpf_by = !!lpf_by;
    return 0;
}
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (sensors & INV_XYZ_COMPASS) {
	    /* com_server_printf("compass enabled\n"); */
        data = AKM_SINGLE_MEASUREMENT;
        user_ctrl |= BIT_AUX_IF_EN;
    } else {
	    /* com_server_printf("compass NOT enabled\n"); */
        data = AKM_POWER_DOWN;
        user_ctrl &= ~BIT_AUX_IF_EN;
    }
    if (st.chip_cfg.dmp_on)
...
int mpu_run_6500_self_test(long *gyro, long *accel, unsigned char debug)
{
    ...
    unsigned short gyro_fsr, sample_rate = 0, lpf, lpf_by  = 0;
...
#ifdef _____AK89xx_SECONDARY  // disable for SPI
...
int mpu_run_self_test(long *gyro, long *accel)
{
    ...
    int result;
    unsigned char accel_fsr = 0, fifo_sensors, sensors_on;
    unsigned short gyro_fsr, sample_rate = 0, lpf = 0, lpf_by = 0;
    unsigned char dmp_was_on;
...
#ifdef AK89xx_SECONDARY
static int mpu_rd_aux_akm(char reg, int len, char *r_data)
{
	char data[1];
	int i;

	for (i = 0; i < len; i++) {
		/* Slave 4 reads from AKM data registers. */
		data[0] = BIT_I2C_READ | st.chip_cfg.compass_addr;
		if (i2c_write(st.hw->addr, st.reg->s4_addr, 1, data))
			return -1;

		data[0] = reg + i;
		if (i2c_write(st.hw->addr, st.reg->s4_reg, 1, data))
			return -1;

		/* Enable slave 4, 1-byte writes. */
		data[0] = BIT_SLAVE_EN;
		if (i2c_write(st.hw->addr, st.reg->s4_ctrl, 1, data))
			return -1;

		while(1) {
			if (i2c_read(st.hw->addr, st.reg->i2c_mst_status, 1, data))
				return -1;
			if (data[0] & (1 << 6))
				break;
			delay_ms(1);
		...
		if (i2c_read(st.hw->addr, st.reg->s4_di, 1, r_data + i))
			return -1;
	}
...
static int mpu_wr_aux_akm(char reg, int len, char *w_data)
{
	char data[1];
	int i;
	...
	for (i = 0; i < len; i++) {
		...
		/* Set slave 4 data. */
		if (i2c_write(st.hw->addr, st.reg->s4_do, 1, w_data + i))
			return -1;
...
static int setup_compass(void)
{
...
#if 0 /* This works only with I2C bus */
    mpu_set_bypass(1);
    ...
    mpu_set_bypass(0);
#else /* This should work for both SPI and I2C */
    mpu_set_bypass(0);
    st.chip_cfg.compass_addr = 0xC;

    mpu_rd_aux_akm(AKM_REG_WHOAMI, 1, data);
    ...
    data[0] = AKM_POWER_DOWN;
    mpu_wr_aux_akm(AKM_REG_CNTL, 1, data);
    delay_ms(1);
    ...
    mpu_rd_aux_akm(AKM_REG_ASAX, 3, data);
    ...
    unsigned short lpa_freq)
{
    ...
    if (lpa_freq) {
	    ...
	    mpu_get_lpf_by(&st.chip_cfg.cache.gyro_lpf_by);
...
lp_int_restore:
    ...
    mpu_set_lpf_by(st.chip_cfg.cache.gyro_lpf_by);
